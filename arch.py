...
AVAILABLE_ARCHS = ["SmolAlexNet", "AlexNet", "VGG16", "VGG19", "SmolResNet50", "ResNet50", "InceptionResNetV2"]
...
def LocalResNet50(input_tensor=None, classes=1000, Normalization=BatchNormalization):
    x = ZeroPadding2D(3)(input_tensor)
    x = Conv2D(64, 7, strides=2, padding="valid")(input_tensor)
    x = Normalization()(x)
    x = ReLU()(x)
...
def get_arch(arg, input_shape, classes, **kwargs):
    ...
    elif arg == "SmolResNet50":
        return SmolResNet50(input_tensor=input_tensor, classes=classes)
